...
public class Ragnarok extends CrawlerAction {

    @Override
    public ArrayList<CrawlerStatResponse> parse(JsonObject... parseables) {
        ArrayList<CrawlerStatResponse> responses = new ArrayList<>();
        responses.add(new CrawlerStatResponse(CrawlerType.PLAYERCHECK, parseables[0].get("online_players").getAsLong()));  
        responses.add(new CrawlerStatResponse(CrawlerType.REGISTERED_PLAYERS, parseables[0].get("registered_players").getAsLong()));  
    ...
    public EndpointType getEndpointType() {
        return EndpointType.RAGNAROKAPI;
    }
}
