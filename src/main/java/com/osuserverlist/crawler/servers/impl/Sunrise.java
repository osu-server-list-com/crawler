...
public class Sunrise extends CrawlerAction {

    @Override
    public ArrayList<CrawlerStatResponse> parse(JsonObject... parseables) {
        ArrayList<CrawlerStatResponse> responses = new ArrayList<>();
        responses.add(new CrawlerStatResponse(CrawlerType.PLAYERCHECK, parseables[0].get("users_online").getAsLong()));  
